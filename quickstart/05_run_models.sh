...
python run_scripts/pred_fp.py --num-workers 0 --labels-name $labels --dataset-name quickstart --model $fp_ckpt --save $output_dir

python run_scripts/retrieval_contrastive.py --num-workers 0 --dataset-name quickstart  --hdf-prefix $hdf_prefix --save $output_dir --labels-name $labels --model $contrast_ckpt --out-name "contrastive_rankings.p" --dist cosine
#

ranking_file="${output_dir}/contrastive_rankings.p"
smi_outs=$output_dir/"smiles_outputs.tsv"
names_file=${hdf_prefix}_names.p
python analysis/retrieval/create_smi_output.py --ranking $ranking_file --save-name $smi_outs --k 5 --names-file $names_file


# Contrastive embed
python run_scripts/embed_contrastive.py --num-workers 0 --model  \
