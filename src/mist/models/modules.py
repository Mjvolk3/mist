...
class FormulaTransformer(nn.Module):
    ...
    def __init__(
        ...
        single_form_encoder: bool = False,
        ...
        **kwargs
        ...
        """__init__.
        Args:
            set_pooling (str) : Name of pooling strategy
                intensity: Weight by intensity of each chem formula
                root: Only take the root node
                mean: Take the mean of all nodes
            use_losses (bool): If true, use losses
            formula_encoder_type (str): Type of checmial formula encoder
            additive_attn (bool): If true, use additive attention
            pairwise_featurization (bool): If true, use pairwise featurization
            num_heads (int): Num attn heads,
            intensities
            recycle_form_encoder (bool): if true, use the same form encoder for
                pairwise as singletons
        """
        ...
        if self.single_form_encoder:
            ...
            dense_formula_encoders = []
            self.formula_dim = utils.ELEMENT_DIM_MASS
            for _type in range(self.num_types):
                ...
                dense_formula_encoders.append(dense_encoder)

            self.formula_encoders = nn.ModuleList(dense_formula_encoders)
        ...
        if not self.use_cls and self.set_pooling == "cls":
            raise ValueError("Must set use-cls flag with set-pooling=cls flag")
    ...
    def forward(self, batch: dict, return_aux=False):
        ...
        if self.single_form_encoder:
            ...
            for encoder_type, encoder in enumerate(self.formula_encoders):
                ind_select = peak_types == encoder_type
                to_embed = form_vec[ind_select]
                if len(to_embed) > 0:
                    # to_embed = to_embed.reshape(batch_size,
                    #                            -1,
                    #                            to_embed.shape[-1])
                    peak_tensor[ind_select] = encoder(to_embed)

        # embedded_types = self.type_embedding_layer(batch['types'])
        # size hidden layer
        peak_tensor = peak_tensor  # + embedded_types
        ...
        if self.pairwise_featurization:
            ...
            pairwise_features = self.pairwise_featurizer(form_diffs)
