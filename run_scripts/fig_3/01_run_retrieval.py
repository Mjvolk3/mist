...
contrastive_retrieval_base = f"python run_scripts/retrieval_contrastive.py --dataset-name {dataset_name} --hdf-prefix data/paired_spectra/{dataset_name}/retrieval_hdf/pubchem_with_csi_retrieval_db --subset-datasets test_only --num-workers 12 --dist-name cosine --gpu"
true_retrieval_ranking = (
    "data/paired_spectra/csi2022/retrieval_hdf/pubchem_with_csi_retrieval_db_ranked.p"
)
lam = 0.3
ensemble_script_base = f"python analysis/retrieval/avg_model_dists.py --lam {lam}"

csi_cos_inds = (
    "data/paired_spectra/csi2022/prev_results/csi_retrieval_cosine_012_ind_found.p"
)
...
for ckpt in Path(contrast_dir).rglob("*.ckpt"):
    cmd = (
        f"{contrastive_retrieval_base} --model-ckpt {ckpt} --save-dir {contrast_outdir}"
    )
    # subprocess.call(cmd, shell=True)
...
ranked_retrieval_extract_base = f"python analysis/retrieval/extract_rankings.py --true-ranking {true_retrieval_ranking} --labels {labels_file}"
out_ind_files = []
for name, j in zip(["fp", "contrast", "merged"], [fp_file, contrast_file, merged_file]):
...
for name, j in zip(
    ...
    summary_cmd = f"python analysis/retrieval/ranking_summary.py --ranking-file {j}"
...
plt_cmd_1 = f"python analysis/retrieval/lineplots.py --retrieval-files {csi_cos_inds} {csi_bayes_inds} {mist_cos_inds} {mist_contrast_inds} --save-dir {save_dir}"
print(plt_cmd_1)
subprocess.call(plt_cmd_1, shell=True)

plt_cmd_2 = f"python analysis/retrieval/retrieval_barplot.py --retrieval-files {csi_bayes_inds} {mist_contrast_inds} --save-name {save_dir / 'barplot.png'} --png"
