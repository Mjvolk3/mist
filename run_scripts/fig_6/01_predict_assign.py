...
for subdir in Path(res_folder).glob("*"):
    ...
    tree_out = Path(
        f"data/paired_spectra/canopus_train/morgan4096_spec_preds_fold_{fold_num}/"
    )
    orig_labels = str(tree_out.parent / "labels.tsv")
    split_file = str(tree_out.parent / "splits" / split_name)

    cmd_2 = f"python data_processing/forward/08_assign_formulae.py  --preds {save_name} --out {tree_out} --num-bins 15000 --upper-limit 1500 --split-file {split_file} --orig-labels {orig_labels}"
    print(cmd_2)
    subprocess.call(cmd_2, shell=True)

    cmd_3 = f"python data_processing/forward/09_export_trees.py --new-spec-dir {tree_out / 'spectra'}"
    print(cmd_3)
    subprocess.call(cmd_3, shell=True)

    cmd_4 = f"python data_processing/forward/10_add_forward_decoys.py --output-folder {tree_out} --labels {tree_out / 'labels.tsv'} --fp-names morgan4096"
    print(cmd_4)
