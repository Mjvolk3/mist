...
for j in fp_dir.glob("ffn_*/preds/*.p"):
    ffn_fp_cmd = f"python run_scripts/retrieval_fp.py --labels-file {labels_file} --fp-pred-file {j} --save-dir {out_ranking_folder / 'retrieval-ffn-fp'} --dist-name cosine --num-workers {num_workers} --hdf-prefix {base_data_dir / 'retrieval_hdf/intpubchem_with_morgan4096_retrieval_db'}"
    print(ffn_fp_cmd)
...
for mist_fp in fp_dir.glob("full_model_*/preds/*.p"):
    mist_fp_cmd = f"python run_scripts/retrieval_fp.py --labels-file {labels_file} --fp-pred-file {mist_fp} --save-dir {out_ranking_folder / 'retrieval-mist-fp'} --dist-name cosine --num-workers {num_workers} --hdf-prefix {base_data_dir / 'retrieval_hdf/intpubchem_with_morgan4096_retrieval_db'}"
    print(mist_fp_cmd)
...
for split_file, mist_fp_ranking in fp_dict.items():
    ...
    python_avg = f"python analysis/retrieval/avg_model_dists.py --first-ranking {mist_contrast_ranking} --second-ranking {mist_fp_ranking} --lam 0.7 --save-name {paired_out}"
    print(python_avg)
...
cmd = f"python analysis/retrieval_ablations/merge_ind_found.py --dir {out_ranking_folder} --out {output.name}"
# subprocess.call(cmd, shell=True)

save_dir = base_dir / "plots"
save_dir.mkdir(exist_ok=True)
save_name = save_dir / "model_ablation.pdf"
cmd = f"python analysis/retrieval_ablations/model_ablation_plot.py --ablation-file {output} --save-name {save_name}"
subprocess.call(cmd, shell=True)
