...
# Make summaries
cmd = f"python analysis/fp_ablations/summarize_data_ablation.py --dir {dirs[0]}"
# subprocess.call(cmd, shell=True)

cmd = f"python analysis/fp_ablations/summarize_model_ablation.py --dir {dirs[1]}"
# subprocess.call(cmd, shell=True)

# Plot data
for metric in ["cos_sim", "ll"]:
    data_ablation_plot = f"python analysis/fp_ablations/data_ablation_plot.py --ablation-file {Path(dirs[0]) / 'fp_pred_summary.tsv'} --metric {metric}"
    subprocess.call(data_ablation_plot, shell=True)

for metric in ["cos_sim", "ll"]:
    data_ablation_plot = f"python analysis/fp_ablations/model_ablation_plot.py --ablation-file {Path(dirs[1]) / 'fp_pred_summary.tsv'} --metric {metric}"
    subprocess.call(data_ablation_plot, shell=True)
