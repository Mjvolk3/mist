...
base_fp = f"python run_scripts/retrieval_fp.py --labels-file {labels_file} --dist-name cosine --num-workers {num_workers} --hdf-prefix {base_data_dir / 'retrieval_hdf/intpubchem_with_morgan4096_retrieval_db'}"
for ckpt in base_dir.rglob("*.ckpt"):
    ...
    contrast_cmd = (
        f"{base_contrastive} --model-ckpt {str(ckpt)} --save-dir {save_dir_contrast}"
    )
    print(contrast_cmd)
    ...
    python_avg = f"python analysis/retrieval/avg_model_dists.py --first-ranking {r1} --second-ranking {r2} --lam 0.3 --save-name {save_name}"
    print(python_avg)
...
cmd = f"python analysis/retrieval_ablations/merge_ind_found.py --dir {out_ranking_folder_joint} --out {output.name}"
# subprocess.call(cmd, shell=True)

save_dir = base_dir / "plots"
save_dir.mkdir(exist_ok=True)
save_name = save_dir / "data_ablation.pdf"
cmd = f"python analysis/retrieval_ablations/data_ablation_plot.py --ablation-file {output} --save-name {save_name}"
subprocess.call(cmd, shell=True)
